...
% Calcolo linea media (spiegazione completa sul report)
CamberPanels = 50000;
...
% Per calcolare l'angolo di progetto è stata calcolata la linea media,
% approssimandola come luogo dei punti equidistanti da dorso e ventre,
% implementando poi una smoothing spline conservativa per ottenere una
% funzione continua per la linea media. Poi è stato eseguito un cambio di
% variabile e una derivazione tramite fnder. Infine si è scelto di usare il
% metodo dei trapezi per l'integrazione della funzione derivata
...
alpha_th = rad2deg(1/pi * trapcomp(eta, dy_eta))

%% (Test per XFOIL) calcolo alpha_0

f = dy_eta .* cos(eta);

alpha_0 = rad2deg(deg2rad(alpha_th) - 1 / pi * trapcomp(eta, f))

% alpha_0 dovrebbe essere tra -3.65° e -3.60°
